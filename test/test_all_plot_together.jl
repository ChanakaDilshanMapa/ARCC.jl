...
plot!(
    ...
    label="FP MO basis", color=:darkblue, linestyle=:solid, linewidth=3,
...
plot!(
    ...
    label="FP AO basis", color=:magenta, linestyle=:dash, linewidth=3,
...
plot!(
    ...
    label="FP Shifted AO basis", color=:orange, linestyle=:dash, linewidth=3,
    # marker=:rect, markersize=5, markerstrokecolor=:orange, markerstrokewidth=1.2
)

color_map_mo = Dict(3=>"#D02000", 5=>"#0BD002", 10=>"#001BDB")
color_map_ao = Dict(3=>"#F08080", 5=>"#80FD80", 10=>"#8080FD")
...
for m_val in (3, 5, 10)
    ...
    plot!(
        ...
        # marker = shape_map_mo[m_val], markersize=markersize_mo[m_val], markerstrokecolor=color_map_mo[m_val], markerstrokewidth=1.0,
    )
    plot!(
        ...
        # marker = shape_map_ao[m_val], markersize=markersize_ao[m_val], markerstrokecolor=color_map_ao[m_val], markerstrokewidth=1.0,
    )
end
...
png_all = joinpath(pkg_root, "test", "convergence_all_in_one_Ethane.pdf")
savefig(p_all, png_all)
@info "Saved single overlay plot (FP + NK) to $(png_all)"
