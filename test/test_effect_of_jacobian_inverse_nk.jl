...
################################################################
x_ink = collect(1:length(newton_post_ink))
x_pnk = collect(1:length(newton_post_pnk))

r0 = if !isempty(newton_pre_ink)
    newton_pre_ink[1]
...
y_ink_plot = vcat([r0], newton_post_ink)
y_pnk_plot = vcat([r0], newton_post_pnk)
x_ink_plot = collect(0:length(newton_post_ink))
x_pnk_plot = collect(0:length(newton_post_pnk))

all_series_y = Float64[]
append!(all_series_y, filter(y -> isfinite(y) && y > 0, y_ink_plot))
append!(all_series_y, filter(y -> isfinite(y) && y > 0, y_pnk_plot))

max_x = max(length(x_ink_plot), length(x_pnk_plot))
...
legend_lw = 2.5
plot!(p_two, [NaN], [NaN]; label="INK", color=:orange, linestyle=:dash, linewidth=legend_lw)
...
pdf_two = joinpath(fig_dir, "effect_of_jacobian_inverse_nk_$(Molecule).pdf")
svg_two = joinpath(fig_dir, "effect_of_jacobian_inverse_nk_$(Molecule).svg")
