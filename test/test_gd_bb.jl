...
base_dir = joinpath(pkg_root, "test/pyscf_data/H2_cc-pvtz");
...
run_gd = gd_runner(new_S, t2, nocc, n_b, Cscf, f, peris, initial_guess, max_iter, α, tol);

# MO case (Identity Transformation)
T_I = Matrix{Float64}(I, n_b, n_b);
θ_final_I, θ_benchmark_I = run_gd(T_I);
...
θ_final_I_plus_P, θ_benchmark_I_plus_P = run_gd(T_I_plus_P);
@test norm(θ_final_I_plus_P - θ_benchmark_I_plus_P) < 1e-8

# AO case (Full Transformation)
T_F = Cscf;
θ_final_F, θ_benchmark_F = run_gd(Cscf);
