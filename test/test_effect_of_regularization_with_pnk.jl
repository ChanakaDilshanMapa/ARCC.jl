...
mp2 = fp_iteration_factory(new_S, mo, nocc, n_b, Cscf, f, peris, initial_guess, 1, tol,shift_canonical; verbose=true);
θ_mp2, _, _ = mp2(T_I);

run_fixed_point = fp_iteration_factory(new_S, mo, nocc, n_b, Cscf, f, peris, θ_mp2, max_iter, tol,shift_canonical; verbose=true);
θ_final_I, θ_benchmark_I, diffs_I = run_fixed_point(T_I);

run_fixed_point_shifted = fp_iteration_factory(new_S, mo, nocc, n_b, Cscf, f, peris, θ_mp2, max_iter, tol,shift_non_canonical; verbose=true);
θ_final_fp_shifted, θ_benchmark_fp_shifted, diffs_fp_shifted = run_fixed_point_shifted(T_I);


run_ink = inexact_newton_factory(new_S, mo, nocc, n_b, Cscf, f, peris, θ_mp2, max_outer_nk, tol, 5);
θ_final_I_l, θ_benchmark_I_l, newton_pre_I_l, newton_post_I_l, num_evals_I_l = run_ink(T_I);
@test norm(θ_final_fp_shifted - θ_final_I_l) < 1e-8

run_nk = nk_solver_factory_with_logs(new_S, mo, nocc, n_b, Cscf, f, peris, θ_mp2, max_outer_nk, tol, 5);
θ_final_nk, θ_benchmark_nk, newton_pre_nk, newton_post_nk, gmres_residuals_nk, num_evals_nk = run_nk(T_I);

run_pnk = preconditioned_nk_solver_factory_with_logs(new_S, mo, nocc, n_b, Cscf, f, peris, θ_mp2, max_outer_nk, tol, 5, 30);
θ_final_pnk, θ_benchmark_pnk, newton_pre_pnk, newton_post_pnk, gmres_residuals_pnk, num_evals_pnk = run_pnk(T_I);

@test norm(θ_final_fp_shifted - θ_final_I_l) < 1e-8
@test norm(θ_final_fp_shifted - θ_final_nk) < 1e-8
@test norm(θ_final_fp_shifted - θ_final_pnk) < 1e-8
################################################################
...
max_x = max(length(diffs_I), length(diffs_fp_shifted), length(newton_post_I_l), maximum(x_pnk))
...
plot!(p_three, [NaN], [NaN]; label="PNK", color=:olive, linestyle=:dashdot, linewidth=legend_lw)
...
pdf_three = joinpath(fig_dir, "effect_of_regularization_$(Molecule)_with_pnk.pdf")
svg_three = joinpath(fig_dir, "effect_of_regularization_$(Molecule)_with_pnk.svg")
...
ene_sfp = corr_ene(int, θ_final_fp_shifted)
ene_ink = corr_ene(int, θ_final_I_l)
ene_nk = corr_ene(int, θ_final_nk)
ene_pnk = corr_ene(int, θ_final_pnk)

@test norm(ene_sfp - ene_ink) < 1e-8
@test norm(ene_sfp - ene_nk) < 1e-8   
@test norm(ene_sfp - ene_pnk) < 1e-8
