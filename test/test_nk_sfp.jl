...
m_diis = 5
run_sfp_plus_diis = fp_iteration_factory_diis(new_S, t2, nocc, n_b, Cscf, f, peris, initial_guess, max_iter, tol, shift_non_canonical, m_diis; verbose=true);
...
r0_nk = if !isempty(newton_pre_nk)
    newton_pre_nk[1]
else
    1.0
end

y_nk_plot = vcat([r0_nk], newton_post_nk)
...
max_x = max(length(x_sfp), length(x_nk_plot), length(x_sfp_plus_diis))
...
legend_lw = 2.5
plot!(p_two, [NaN], [NaN]; label="SFP", color=:darkgreen, linestyle=:solid, linewidth=legend_lw)
plot!(p_two, [NaN], [NaN]; label="NK", color=:orange, linestyle=:dash, linewidth=legend_lw)
plot!(p_two, [NaN], [NaN]; label="m=$(m_diis) SFP+DIIS", color=:darkblue, linestyle=:dot, linewidth=legend_lw)
