...
function fixed_point_factory(new_S, t2, nocc, n_b, Cscf, f, peris, initial_guess, max_iter, tol, shift; verbose=true)
    return function (T)
        Tbar = T_bar(T, new_S)
        slice = make_slices(Cscf, T, Tbar, nocc, n_b)
        proj = make_projectors(slice)
...
export preconditioned_nk_factory

function preconditioned_nk_factory(new_S, t2, nocc, n_b, Cscf, f, peris, initial_guess, max_outer, tol, m, shift)
    return function (T)
        Tbar = T_bar(T, new_S)
        slice = make_slices(Cscf, T, Tbar, nocc, n_b)
        proj = make_projectors(slice)
        ...
        θ_final, num_residual_evals = preconditioned_newton_krylov(initial_guess, build_residual, elt, shift; 
        tol=tol, max_outer=max_outer, m=m, verbose=true)
        return θ_final, θ_benchmark, num_residual_evals
    end
end

export preconditioned_nk_factory_with_logs

function preconditioned_nk_factory_with_logs(new_S, t2, nocc, n_b, Cscf, f, peris, initial_guess, max_outer, tol, m, shift)
    return function (T)
        Tbar = T_bar(T, new_S)
        slice = make_slices(Cscf, T, Tbar, nocc, n_b)
        proj = make_projectors(slice)
        ...
        θ_final, newton_pre, newton_post, gmres_residuals, num_residual_evals = preconditioned_newton_krylov_with_logs(initial_guess, build_residual, elt, shift; 
        tol=tol, max_outer=max_outer, m=m, verbose=true)
